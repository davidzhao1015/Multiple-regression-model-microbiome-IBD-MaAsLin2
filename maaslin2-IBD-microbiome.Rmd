...
all_results_sig <- all_results %>% 
  mutate(whether_sig = case_when(coef >= 0.3 & qval <= 0.05 ~ "Increase",
                                 coef <= -0.3 & qval <= 0.05 ~ "Decrease",
                                 abs(coef) < 0.3 | qval >0.05 ~ "No change")) 

# add label 
all_results_sig$relabel <- NA

all_results_sig$relabel[all_results_sig$whether_sig != "No change"] <- all_results_sig$feature[all_results_sig$whether_sig != "No change"]

# set personal color palate 
mycolors <- c("blue", "red", "black")
names(mycolors) <- c("Decrease", "Increase", "No change") 
