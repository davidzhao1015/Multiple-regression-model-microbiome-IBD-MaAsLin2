...
The scripts was built with R `r getRversion()`.
...
library(ggplot2)
```
The information regarding the system, R, R studio and loaded libraries was here `r sessionInfo()`
...
```{r microbiom data, message=FALSE, results='hide', collapse=TRUE}
...
head(df_input_metadata, 5)
```


## Exploratory data analysis
Generate descriptive statistics for microbiome relative abundance data, calculating mean, standard deviations\ 
and rate of missing values 
...
```{r build a function, include=TRUE} 
...
## Fit a mutiple regression model on microbiome and clinical variables
